...
const Add = ({ token }) => {
  ...
  const handleUpload = () => {
    ...
    reader.onload = async (e) => {
      ...
      for (const product of products) {
        ...
        imageName.forEach((imgKey, index) => {
          const fileMatch = imageMap[`${imgKey}.png`]; // only check for .png

          if (fileMatch) {
            formData.append(`image${index + 1}`, fileMatch); // image1, image2, ...
          } else {
            console.warn(`Image not found for: ${imgKey}.png`);
          }
        })
        ...
        try {
          ...
          console.error("Upload failed for", product.name, err.response?.data || err.message);
        }
      }
  ...
  return (
    <div>
      ...
      <div className='flex flex-col items-start mb-[50px] gap-2'>
        ...
        <div className='flex gap-4'>
          <input className="w-[200px] bg-gray-200" type="file" accept='.json' onChange={(e) => setFile(e.target.files[0])} />
          <input className="w-[200px] bg-gray-200" type="file" accept="image/*" onChange={handleImageChange} multiple directory="" webkitdirectory="" />
        </div>

        <div>
          <button className="bg-blue-200 rounded px-3 py-1" onClick={handleUpload}>Upload</button>
        </div>
